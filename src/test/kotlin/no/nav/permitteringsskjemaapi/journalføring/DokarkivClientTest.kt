...
class DokarkivClientTest {
    ...
    fun oppretterJournalpostMedM2MToken() {
        ...
        server.expect(requestTo("/journalpost?forsoekFerdigstill=true"))
            ...
            .andExpect(jsonPath("$.journalfoerendeEnhet").value("9999"))
            .andExpect(jsonPath("$.behandlendeEnhet").value(behandlendeEnhet))
            .andExpect(jsonPath("$.dokumenter[0].brevkode").value(SkjemaType.INNSKRENKNING_I_ARBEIDSTID.innsendtPermitteringJournalpost.dokument.brevkode))
            .andExpect(jsonPath("$.dokumenter[0].tittel").value(SkjemaType.INNSKRENKNING_I_ARBEIDSTID.innsendtPermitteringJournalpost.tittel))
